  ...
  const loadFirebaseCases = async () => {
    ...
    try {
      ...
      if (fbCases.length > 0) {
        ...
        setSyncStatusMsg(`✅ ('robot rp', 'robotik rp', 'robotik radikal prostatektomi') etiketli ${addedCount} vaka Firebase'den çekildi.`);
      } else {
        ...
        {upcomingOps.length === 0 ? (
          <div className="col-span-full bg-white p-12 rounded-2xl border border-slate-200 text-center space-y-3">
            ...
            <p className="text-xs text-slate-500 max-w-md mx-auto">
              Yukarıdaki <strong>"Gelecek Ameliyatları Çek (Bugün ve Sonrası)"</strong> butonuna basarak Firebase projenizden <i>'robot rp'</i>, <i>'robotik rp'</i> veya <i>'robotik radikal prostatektomi'</i> etiketli vakaları listenize çekebilirsiniz.
            </p>
          </div>
        ) : (
          upcomingOps.map(op => {
